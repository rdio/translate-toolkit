...
class TranslatePage(pagelayout.PootlePage):
  ...
  def __init__(self, project, subproject, session, argdict):
    ...
    translated, total = self.translationproject.getpostats(self.pofilename)
    self.links.addcontents(widgets.Division("%d/%d translated" % (translated, total), None, {"class":"side"}))
